...
<template>
  <div class="space-y-2">
    ...
    <section
      class="animate-settle flex flex-col gap-2.5 rounded-lg border border-caution/35 bg-caution-soft px-3.5 py-2.5 sm:flex-row sm:items-center sm:gap-3"
      :aria-label="`Allow ${prompt}`"
    >
      <p class="min-w-0 flex-1 break-words font-mono text-sm text-ink">
        Allow {{ prompt }}
      </p>

      <div class="flex shrink-0 items-center gap-4 self-end sm:self-auto">
        <button
          type="button"
          class="min-h-10 text-sm font-medium text-[oklch(55%_0.16_45)] transition-opacity duration-150 hover:opacity-80"
          @click="emit('allow')"
        >
          Allow
        </button>
        <button
          type="button"
          class="min-h-10 text-sm font-medium text-danger transition-opacity duration-150 hover:opacity-80"
          @click="emit('deny')"
        >
          Deny
        </button>
    ...
    <p v-if="hasPreviewLink" class="animate-rise pl-0.5 text-sm">
      <button
        type="button"
        class="min-h-10 font-medium text-accent underline-offset-2 transition-colors duration-150 hover:text-accent-hover hover:underline"
        @click="openPreview"
      >
        Preview campaign
      </button>
    ...
    >
      ...
      <span class="mt-1 block text-pretty">
